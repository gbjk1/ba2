...
as promising solutions. Apart from paid subscriptions they both offer free, self hosted versions and therefore qualify for the setup. The decision for Teleport comes down to the fact that Teleport natively provides even more granular access control through ABAC. For example, it allows for specifying weekdays and time ranges for accessibility of certain resources. 
Hoop.dev implements only session-based time constraints and requires additional plugin handling in order to achieve mature ABAC. 
...
Teleport requires integration to MFA from the very start. When accessing the Teleport UI as well as when using the tsh commandline client, users are prompted for One-Time password tokens (OTP). 
\newline
teleport-agent-namespace, grpahic with proxying

\begin{figure}[htbp]
  \centering
      \includegraphics[width=14cm]{./images/teleportarch}
  \caption{Teleport Kubernetes Setup [source: author]}
  \label{fig:teleportarch}
\end{figure}
user login example
\newline
time based policy as example of ABAC
\newline
