...
clients such as kubectl and helm are using a static client keys and certificates placed under .kube/config of the client machine. These credentials are permanently valid until they are manually invalidated.  
\newline
\newline
...
location or network status, as aforementioned in section 2.1. 
...
certain period, contrasting sharply with the long-term validity of static credentials. This strategy is consistent with the Zero Trust ethos of minimizing vulnerabilities by avoiding the use of persistent secrets. 
...
thorough monitoring for potential security threats, as mentioned in NIST800-207, and as summarized in chapter 2. 
...
This method strenghtens security by ensuring that users and devices are only granted access based on their immediate needs, reducing the likelihood of unauthorized access or lateral movement within the network.
...
\subsubsection{title}
Touch on OIDC, MFA, auditing sessions, minimal access, encryption, etc when accessing the cluster and how Teleport is cool
...
a sidecar approach are sophisticated routing capabilities, separation of concerns and enhanced security. 
cite[dynatrace]
...
Talk about how cool Istio is and how it takes network/authetication handling away from code deployed in containers, separation of concern.
...
HERE OIDC
Touch on OIDC and its combination with Istio
