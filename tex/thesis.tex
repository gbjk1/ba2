...
\cleardoublepage
\chapter*{Abstract}
Kubernetes is a highly customizable container orchestration platform which is progressively more relied on in production systems. 
For this reason, securing this platform is of utmost importance. This thesis investigates how the security model Zero Trust, which 
which aspires to replace traditional security approaches, can be applied to a Kubernetes environment. 
The NIST Special Publication 800-207 "Zero Trust Architecture" is used as the base reference.
Technologies suitable for the Zero Trust approach are evaluated and compared.
Finally, Teleport, Istio and Keycloak are established as solid solutions for the Zero Trust Kubernetes environment. 
Concrete configurations are given and the final Kubernetes setup is demonstrated.

% --- German Abstract ----------------------------------------------------
\cleardoublepage
\chapter*{Kurzfassung}
Kubernetes ist eine hochgradig anpassbare Container-Orchestrierungsplattform, die in Produktionssystemen immer häufiger zum Einsatz kommt. Aus diesem Grund ist die Absicherung dieser Plattform von enormer Wichtigkeit.
In dieser Arbeit wird untersucht, wie das Sicherheitsmodell Zero Trust, das traditionelle Sicherheitsansätze ersetzen soll, auf eine Kubernetes-Umgebung angewendet werden kann. Die
NIST Special Publication 800-207 „Zero Trust Architecture“ wird als Basisreferenz verwendet. Technologien, die sich für den Zero Trust Ansatz eignen, werden evaluiert und verglichen. 
Schließlich werden Teleport, Istio und Keycloak als solide Lösungen für die Zero Trust Kubernetes-Umgebung etabliert. Konkrete Konfigurationen werden angegeben und das endgültige Kubernetes-Setup wird demonstriert.
...
The increasing necessity for scalability and flexibility made organizations tran-
sition from monolithic architectures to so called microservices. Microservices are smaller, separated but
loosely coupled software units implementing one  responsibility of the larger system at hand.
The release of the Docker container platform in 2013 had a major impact on making
...
trust architecture plan" \cite{nist800207}
\newline
\newline
...
The Policy Engine (PE) decides whether or not to allow access to a resource for a particular client.
...
The policy engine also documents its approval or rejection. 
The policy administrator then puts the decision into action.
\newline
\cite{nist800207}

\subsubsection{Policy Administrator}
The Policy Administrator is the component handling initiation as well as termination of the communication line between a resource and a subject. 
Any session-specific authentication, credential, or token 
that a client uses to get access to an enterprise resource is generated by it. It is related to the PE and 
depends on its final determination of whether to approve or reject a session. The PA sets up the Policy Enforcement Point (described below) to allow the session 
to begin if the request has been authenticated successfully. The PA notifies the PEP to terminate the connection 
...
resource. The PEP interacts with the PA in order to transmit requests and
...
themselves inside the reserved ’kube-system’ namespace. \cite{k8scomponents}
\newline
\begin{figure}[htbp]
  \centering
...
certificates placed under .kube/config of the client machine. However, access to the cluster can also happen by means of a dedicated authentication-proxy solution. \cite{kubeconfig}
\newline
\newline
...
achieved through Role, ClusterRole, RoleBinding and ClusterRoleBinding resources. \cite{kubeconfig} \cite{kubeconfigrbac} This method fundamentally lacks the features needed to come closer to a ZT approach. Kubernetes itself does not offer one centralized way of creating, issuing and rotating these credentials. They
...
offering strong security features for ZT. It can act as an access gateway in front of Kubernetes clusters. However, this feature is not free and open source software and cannot be self-hosted. \cite{okta} It was therefore not taken into further consideration. Hoop.dev and Teleport remain 
...
focusing instead on verifying the identity of users rather than their network location, which is central to the Zero Trust philosophy, as aforementioned in section 2.1. \cite{endpassword}
...
certain period, contrasting sharply with the long-term validity of static credentials. This strategy is consistent with the Zero Trust ethos of minimizing vulnerabilities by avoiding the use of persistent secrets. \cite{endpassword}
...
thorough monitoring for potential security threats, as mentioned in NIST800-207, and as summarized in chapter two. \cite{teleportrbac}

\subsubsection{Public-Facing Proxy}
...
This method strenghtens security by ensuring that users and devices are only granted access based on their immediate needs, reducing the likelihood of unauthorized access or lateral movement within the network. \cite{teleportproxy}

\subsubsection{Multi-Factor Authentication}

Multi-Factor Authentication (MFA) is a security measure that requires users to provide two or more verification factors to access a resource such as an application or online account. These factors can include something the user has knowledge of (passwords), something 
the user is in possession of (smartphones for example), or a biological characteristic (fingerprint, facial recognition). By requiring multiple types of verification, MFA significantly increases the difficulty for unauthorized individuals to gain access, even if one 
...
\begin{figure}[htbp]
  ...
  \caption{Teleport installation [source: Teleport \cite{teleportinstall}]}
  \label{fig:teleport}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Teleport agent installation [source: Teleport \cite{agentinstall}]}
...
Teleport requires integration to MFA from the very beginning. When accessing the Teleport UI as well as when using the tsh commandline client, users are prompted for One-Time password tokens (OTP) issued by Authy. \cite{authy}
\newline
...
\begin{figure}[htbp]
  ...
  \caption{Teleport Kubernetes Setup [source: Teleport \cite{agentinstall}]}
...
a sidecar approach are sophisticated routing capabilities, separation of concerns and enhanced security. \cite{awsmesh}
cite[dynatrace]

\begin{figure}[htbp]
  \centering
      \includegraphics[width=11cm]{./images/mesh}
  \caption{Service Mesh visualization [source: RedHat \cite{redhatmesh}]}
...
outlined in section 2.2.2 "Core Cluster Objects". Using the script shown in Figure 3.14, the following installation steps are executed.
...
\begin{figure}[htbp]
  ...
  \caption{Installing Istio components [source: Istio \cite{helmistio}]}
...
Istio's architecture including its Ingressgateway, sidecars, and control plane is depicted in Figure 3.15. \cite{istioingress}

\begin{figure}[htbp]
  \centering
      \includegraphics[width=13cm]{./images/istioarchitecture}
  \caption{Istio architecture [source: Istio \cite{istioarch}]}
...
database administrator might have a role that allows access to certain databases and system maintenance tools. \cite{mohamed2022authorization}
\newline
...
certain servers only during business hours and exclusively from within the corporate network. This degree of granularity makes ABAC effective in environments where access requirements are complex and highly variable. \cite{mohamed2022authorization}
\newline
...
\begin{itemize}
	\item keycloak-service.keycloak.svc.cluster.local/realms/nebhis/protocol/openid-connect/token. This endpoint is consumed by applications requesting an OIDC token, to be further used to authenticate to other applications in the cluster. \cite{keycloak}
	\item keycloak-service.keycloak.svc.cluster.local/realms/nebhis/protocol/openid-connect/certs. This endpoint is consumed by the Istio service mesh. The sidecar intercepts incoming traffic to an application pod, extracts the token from the 'Authorization' HTTP header, 
        and validates it using the PKI information obtained under this endpoint. \cite{keycloak}
\end{itemize}
...
due to fast changing landsacape IT security and Kubernetes. Thus, future work might build on top of this setup and concern itself with the exact critiques laid out above. \cite{ai_threat_detection_2024}
