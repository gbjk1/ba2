...
To achieve Zero Trust Principles, following technologies need to present. 
...
Such access must happen in a secure, granular and audited fashion in order for it to fulfill Zero Trust requirements. The default way of accessing the cluster is done by using static client keys and 
...
\newline
\newline
Kubeconfig, Teleport, Okta and Hoop are ways of accessing the cluster which were taken into consideration for comparison. Kubeconfig means accessing the cluster thorugh a manually issued kubeconfig file. Teleport, Okta and Hoop were identified as authentication-proxy 
...
\begin{itemize}
	...
	\item Offers Role Based Access Control
	\item Offers Attribute Based Access Control
	\item Provides MFA
	\item Maturity of the project, release status 
\end{itemize}

Out of the box, Kubernetes offers its standard 

These credentials are permanently valid until they are manually invalidated. Kubernetes itself does not offer one centralized way of creating, issuing and maitaining and invalidating these credentials, would neeStandard Kubeconfig not in accordance with ZT 
