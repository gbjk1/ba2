...
gateway, an open-source firewall and based on FreeBSD, which controls external traffic coming into 
...
entering or leaving the internal network, playing a critical role in securing and directing network traffic. As this thesis focuses on the Kubernetes environment, and as OPNsense is not directly 
...
These VMs serve various roles within the network, hosting different services and agents required for the Kubernetes Cluster. A static IP assignment was chosen to ensure consistent network addressing, 
...
This choice of operating system underscores a preference for a robust, community-supported platform capable of handling the demands of modern networked applications. 
With 8GB of RAM and 4 CPU cores allocated to each VM, they are well-equipped to run Kubernetes.

\section{Bootstrapping cluster with kubeadm}
