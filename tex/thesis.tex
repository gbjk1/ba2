...
\newline
\newline
Kubeconfig, Teleport, Okta and Hoop are ways of accessing the cluster which were researched for suitability for the setup of this thesis. Kubeconfig means accessing the cluster thorugh a manually issued kubeconfig file. Teleport, Okta and Hoop were identified as authentication-proxy 
solutions to the cluster, aiming to implement ZT. To identify the most appropriate solution for a ZT approach, following criteria was established: 
\begin{itemize}
...
are permanently valid until they are manually invalidated, creating a long-lived, single point of compromise. For this reason, authentiction proxies such as Teleport, Okta, and Hoop are more appropriate solutions for cluster access. Okta is a mature identity and access management platform
offering robust security features for ZT. It can act as an access gateway infront of Kubernetes clusters. However, this feature is not free and open source software and cannot be self-hosted. It was therefore not taken into further consideration. Hoop.dev and Teleport remain 
...
Kubernetes configuration are replaced with concrete values from a dedicated values file. The script in figure 3.9... 
\begin{enumerate}
...
Teleport requires integration to MFA from the very start. When accessing the Teleport UI as well as when using the tsh commandline client, users are prompted for One-Time password tokens (OTP) issued by Authy. 
\newline
...
"teleport-admin" authenticates to the Teleport cluster "teleport.nebhis.lcl", hosted on 192.168.20.2. This Teleport proxy acts as an intermediary infront of the Kubernetes cluster, enforcing MFA and mTLS. 
Running "tsh kube login nebhis-kubernetes" puts the registered cluster "nebhis-kubernetes" in the current context of the kubectl commandline client by modyfying the kubeconfig file under the .kube/config 
...
According to the documentation, mTLS and access logging is provided by all solutions. Audit logging is not natively supported by any of the four solutions. Therefore it comes down to the degress of granularity offered by the service meshes.
To evaluate granularity, adjectives 'weak', 'limited' and 'strong' are used. Granularity of traffic routing pertains to how detailed cluster traffic can be controlled as it moves from its origin pod to its destination pod. For traffic routing, each service mesh
...
traffic only based on service's mTLS identity. It does not offer regulating traffic based on definite IP addresses, native Kubernetes service accounts, namespaces, HTTP method or OIDC credentials. Consul leverages its service-intentions, service-resolvers, 
...
inject sidecars. Rather, it leverages eBPF Linux kernel, which allows for running custom modules in kernel space of the node. This eBPF program, the Cilium agent, implements rules specified in CiliumNetworkPolicy resources. Cilium natively supports the concept of 
...
outlined in section 2.2.2 "Core Cluster Objects". Using the script shown in 3.14... 
...
Istio's architecture including its Ingressgateway, sidecars, and control plance is depicted in figure 3.15.
...
their roles. This model simplifies administration by enabling system administrators to manage permissions via roles rather than on an individual user basis. For instance, a software developer might have a role that permits access to certain repositories, whereas a
...
\newline
Attribute-Based Access Control (ABAC) is a more adaptable and flexible access control model that grants access based on a combination of attributes pertaining to the user, the resource, and the environment. These attributes can include user details (such as department, job title, or clearance level), 
...
However, this thesis comprehensively covers how the core ideas of ZT, for example the elimination of implicit trust between participating parties, 
can be achieved. If an attacker was to gain access to a pod, they would be significantly restricted in their actions by network segmentation as well as Istio's 
traffic controlling policies tightly integrated with Keycloak's Identity and Access Management. 

the only caveat of istio - istio might impact performance/resource strain on the cluster because of sidecars in every pod 
...
\newline
There can be even more focus placed on the ZT tenet of auditing. Log collector and aggregator might be to standardize the output stream of audit logs provided by Teleport, Istio and Keycloak,
summarize and report them. 
\newline
An improvement that could be made to the setup is the usage of Infrastructure as Code and GitOps. With Iac, the Kubernetes VMs could be managed and provisioned using machine-readable
configuration files, rather than manually issuing interactive commands or using bash scripting. GitOps is a set of practices that uses Git repositories as the single source of truth for 
declarative infrastructure and applications, enabling automated deployments and operational tasks through pull requests and continuous delivery pipelines. Tools like Flux and ArgoCD can 
...
\newline
continuous scanning and kyverno
\newline
As one can see, there is always room for improvement there will always be ways to elevate the security standard of a Kubernetes cluster and an infrastructure in general. 
cant expect to have 100\% ZT. also due to fast changing landsacape and technologies and new findings 
\newline
...
\newline
Thus, future work might build on top of this setup and concern itself with exactly critiques laid out above, thoroughly implementing secret management, continuous scanning and log aggregation
...
- It was shown how a cluster was created from scratch and modified according the ideas of Zero Trust 

this thesis - Thesis gives insight into what is possible and presents simple examples, in a production one must evaluate themself how fine grained the policies must be, meaning to find the balance between developer restriction and security
- plain network communication is blocked bt network policies, and where network-wise a service is reachable, one must present a token 

- there is no checklist which unequivocally decides if something is ZT, something can have ZT characteristics, and in this thesis it was attempted to achieve many of these characteristics. most of them were tried to be imolemented here 

* significance of this work lies in its practical demonstration of applying Zero Trust principles to a Kubernetes environment, providing a blueprint for enhancing security in modern containerized applications

ZERO TRUST is not a one size fits all solution blindly applied and never looked at again, its a continuous process assessing the security posture of the environment and making changes accordingly, attitude of everyone involved
